...
> 5. 어떤 $a \in A$에 대하여, $a <x$ 라고 가정하면, $L$의 정의상 $a$는 $L$의 상계이다. 
> 6. 그런데 $x$는 $L$의 [[AN-0101 상한의 두 가지 정의 (Two Definitions of Supremum)|최소 상계]]이므로 $x\leq a$이어야 한다. 이는 가정($a<x$)에 모순이다.
> 7. 귀류법에 의해 모든 $a \in A$에 대하여 $a \geq x$이다. (즉, $x$는 $A$의 하계이다.)
> 8. $x$는 모든$l\in L$($A$의 하계)에 대하여, $l\leq x$이므로 $x=\text{inf }A$이다.
> 9. 따라서 Corollary 1이 성립한다.
